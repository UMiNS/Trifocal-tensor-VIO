...
for step = 1:M
   ...
   if newData
       ...
       elseif step == 2
          ...
          im2 =double(frame2);
          [x2, y2,num2] = get_interest_points(im2,ws);
          locs2 = [x2 y2];
          
          features1 = get_features(im1, x1, y1, ws);
          features2 = get_features(im2, x2, y2, ws);
  
          [matchTab12, confidences12] = match_features(features1, features2);
          %uv12 = get_uv(matchTab12,locs1,locs2);
          [U12, V12] = LK1(im1, im2, ws, locs1);
          uv12 = [U12 V12];
          % show optical flow 
          %figure; showOF(I1, locs1, U, V);
          ...
          im3 =double(frame3);
          [x3, y3,num3] = get_interest_points(im3,ws);
          locs3 = [x3 y3];
          
          features2 = get_features(im2, x2, y2, ws);
          features3 = get_features(im3, x3, y3, ws);
  
          [matchTab23, confidences23] = match_features(features2, features3);
          %uv23 = get_uv(matchTab23,locs2,locs3);
           [U23, V23] = LK1(im2, im3, ws, locs2);
           uv23 = [U23 V23];
          % show optical flow 
          %figure; showOF(I2, locs2, U, V);
       end
   end
               ...
               for i = 1:size( lmk, 2 )
                   if lmk{i}.matchID > 0
                       if matchTab12(lmk{i}.matchID,1) > 0
                           ...
                           lmk{i}.uv(:,2) = [ uv12(i,2); uv12(i,1); ];
                           lmk{i}.matchID = matchTab12(lmk{i}.matchID,2);
                       else
                           lmk{i}.matchID = 0;
                       end
                   end
               ...
               for i = 1:len %size( lmk, 2 )
                   if lmk{i}.matchID > 0
                       if matchTab23(lmk{i}.matchID,1) > 0
                           ...
                           lmk{i}.uv(:,3) = [ uv23(i,2); uv23(i,1); ];
                           lmk{i}.matchID = matchTab23(lmk{i}.matchID);
                       else
                           lmk{i}.matchID = 0;
                       end
                   end
